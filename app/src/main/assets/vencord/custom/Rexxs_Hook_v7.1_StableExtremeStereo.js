...
/* ═══════════════════════════════════════════════════════════
   REXX'S HOOK v6.8 — REAL 100M GAIN · MULTI-STAGE LOUD · 50 VOICE FX
   ...
   - Gain: 1× to 1,000,000,000,000,000,000,000,000× (log scale)
...
/* ── RTC AUDIO ENCODER: never add our own bitrate ceiling ── */
if(window.RTCRtpSender && RTCRtpSender.prototype.getParameters &&
   RTCRtpSender.prototype.setParameters){
  var _rhSetParams = RTCRtpSender.prototype.setParameters;
  RTCRtpSender.prototype.setParameters = function(params){
    try{
      if(params && this.track && this.track.kind==='audio' &&
         params.encodings){
        params = Object.assign({}, params);
        params.encodings = params.encodings.map(function(enc){
          var e = Object.assign({}, enc);
          delete e.maxBitrate;
    ...
    return _rhSetParams.call(this, params);
  };
}
...
/* ── STATE ──────────────────────────────────────────────── */
var S = {
  // Gain (0-100 log slider → 1× to 1e24×)
  gain:0,
  // Distortion
...
window.AudioContext = function(o){
  var ctx = new _NAC(Object.assign({latencyHint:'interactive'}, o||{}));
...
navigator.mediaDevices.getUserMedia = function(constraints){
  if(constraints && constraints.audio){
    ...
    /* Keep Discord/browser's own device constraints. Only disable processing
       that can pump/cut the voice, and request stereo as an IDEAL value so
       Android devices that expose a mono mic do not reject getUserMedia. */
    a.echoCancellation = false;
    a.noiseSuppression = false;
    a.autoGainControl  = false;
    /* Disable automatic voice processing without forcing a hardware format.
       Do NOT require stereo here: many Android mics are physically mono.
       Stereo is created later by the DSP/output graph. */
    delete a.sampleRate;
    delete a.channelCount;
    a.echoCancellation = false;
    a.noiseSuppression = false;
    a.autoGainControl  = false;
  ...
  return _gum(constraints).then(function(rawStream){
    ...
    return _gum({audio:true, video:constraints && constraints.video ? constraints.video : false});
  });
};
...
var WK = `class RHv6 extends AudioWorkletProcessor {
  ...
  constructor(){
    ...
    /* Always-stereo voice bed. A tiny decorrelation delay is used only to
       create a stable side signal when Android supplies a mono mic. It is
       deliberately subtle so speech stays centered and does not phase-warp. */
    const sLen = (sr*.012|0)+8;
    this.sBuf=new Float32Array(sLen); this.sBufLen=sLen; this.si=0;
  }
  ...
  process(inp, out, p){
    ...
    const Gval = Math.max(1, Math.min(1e24, G));
    ...
    for(let i=0;i<len;i++){
      ...
      /* ─ ALWAYS STEREO ─
         Keep existing L/R when available, and add a very small delayed side
         component so a mono Android microphone becomes an actual 2-channel
         stream. No compressor, limiter, normalization, or dB reduction. */
      const mid0 = (L + R) * 0.5;
      const side0 = (L - R) * 0.5;
      this.sBuf[this.si] = mid0;
      const sdIdx = (this.si + this.sBufLen - Math.max(1,(sampleRate*0.004)|0)) % this.sBufLen;
      const decor = this.sBuf[sdIdx] || 0;
      this.si = (this.si + 1) % this.sBufLen;
      const stereoSide = side0 + decor * 0.055;
      L = mid0 + stereoSide;
      R = mid0 - stereoSide;
      ...
      /* Keep the AudioWorklet alive at extreme gain. JavaScript numbers can
         exceed Float32, but AudioWorklet output cannot. Never write Infinity
         or NaN into the output buffer: that can make the downstream RTC track
         effectively silent. Normal values remain completely uncapped. */
      const F32 = 3.402823466e38;
      if (!isFinite(L)) L = L < 0 ? -F32 : F32;
      else if (Math.abs(L) > F32) L = L < 0 ? -F32 : F32;
      if (!isFinite(R)) R = R < 0 ? -F32 : F32;
      else if (Math.abs(R) > F32) R = R < 0 ? -F32 : F32;
      ...
      for(let i=0;i<len;i++){
        o0[0][i]=Number.isFinite(i0[0][i]) ? i0[0][i] : 0;
        if(o0[1]) o0[1][i]=Number.isFinite(i0[1] ? i0[1][i] : i0[0][i]) ? (i0[1] ? i0[1][i] : i0[0][i]) : 0;
      }
      return true;
    }
  }
...
var CHAIN = {
  ...
  loadWorklet: function(ctx, src, dest){
    ...
    ctx.audioWorklet.addModule(URL.createObjectURL(blob)).then(function(){
      ...
      /* Disconnect passthrough, insert worklet + postGain. Only this source
         is rewired; the destination track itself stays alive, so Discord does
         not lose the microphone while the DSP comes online. */
  ...
  update: function(){
    ...
    var gainLin = S.gain<=0 ? 1 : Math.pow(10, S.gain/100*8); // 1 to 100,000,000
    if(!isFinite(gainLin)) gainLin = 100000000;
    var gBoost = (+S.g1||1) * (+S.g2||1) * (+S.g3||1) * (+S.g4||1) * (+S.g5||1);
    if(!isFinite(gBoost) || gBoost < 1) gBoost = 1;
    set('gain', Math.min(1e16, gainLin * gBoost));
...
var UI = {
    ...
    w.innerHTML=
      ...
      '<div class="P on" data-pane="main">'+
        '<div class="C"><div class="C-row"><span class="C-lbl">🔥 Gain</span><span class="C-val" id="v-gain">1×</span></div>'+
          ...
          '<div class="C-hint">log scale: 0=1× · 50=1e12× · 100=1e24× · raw/no limiter</div></div>'+
  ...
  bind: function(){
    ...
    /* ── GAIN (log scale 0-100 → 1× to 1e24×) ── */
