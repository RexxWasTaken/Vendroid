...
(() => {
  ...
  function makeEngine(inputStream) {
    const __liveTracks = inputStream.getAudioTracks().filter(t => t.readyState === "live");
    if (__liveTracks.length === 0) {
      throw new Error("REXX COVER: upstream stream has no live audio track");
    }

    /* CONTEXT STRATEGY (final):
       Sharing Hook's context forces Hook's AudioWorklet and Cover's whole
       DSP graph to fit inside ONE shared ~3ms render-quantum budget on a
       single thread. Now that Mix defaults to 100 (needed so sliders like
       Ultra Gain work immediately, independently, without also touching
       Mix), Cover's wet chain is connected by default — so sharing a
       thread with Hook's worklet means they permanently compete for the
       same budget, which is what produced "only one works at a time" and
       the overall slowness.
       Fix: Cover gets its OWN independent context again (its own thread,
       its own budget — no competing with Hook's worklet at all). The
       earlier concern with a separate context (cross-context clock drift
       causing delayed/sped-up audio) is addressed differently now: each
       individually expensive stage (the harmonic shapers, the exciter
       send, the compressor/limiter) is dynamically disconnected from the
       graph unless its own slider is actually above 0 (see the bypass
       functions below), so Cover's own context has very little real work
       to do at rest and isn't the kind of heavily-loaded graph that tends
       to drift/underrun in the first place. */
    const __prevDiscordCtx = window.DiscordContext;
    const ctx = new NativeAC({ latencyHint: "interactive" });
    ...
    // exciter dry always connected; wet send/return is dynamically bypassed
    warmthShelf.connect(exciterSum); // dry (always on, unity)
    let exciterActive = false;
    function setExciterBypass(shouldRun) {
      if (shouldRun === exciterActive) return;
      try {
        if (shouldRun) {
          warmthShelf.connect(exciterHPF);
          exciterHPF.connect(exciterShaper);
          exciterShaper.connect(exciterWetGain);
          exciterWetGain.connect(exciterSum);
        } else {
          try { warmthShelf.disconnect(exciterHPF); } catch (_) {}
          try { exciterHPF.disconnect(exciterShaper); } catch (_) {}
          try { exciterShaper.disconnect(exciterWetGain); } catch (_) {}
          try { exciterWetGain.disconnect(exciterSum); } catch (_) {}
        }
      } catch (_) {}
      exciterActive = shouldRun;
    }

    // warmth/saturation/soft-clip trio: dynamically bypassed as a group
    // when all three are at 0%, since that's the common "not using any
    // harmonic coloration" resting state.
    const dynOut = ctx.createGain();
    dynOut.gain.value = 1;
    let shaperTrioActive = false;
    function setShaperTrioBypass(shouldRun) {
      if (shouldRun === shaperTrioActive) return;
      try {
        if (shouldRun) {
          exciterSum.disconnect(ultraGain);
          ...
          try { exciterSum.disconnect(warmthShaper); } catch (_) {}
          try { warmthShaper.disconnect(satShaper); } catch (_) {}
          try { satShaper.disconnect(softClipShaper); } catch (_) {}
          try { softClipShaper.disconnect(ultraGain); } catch (_) {}
          exciterSum.connect(ultraGain);
        }
      } catch (_) {}
      shaperTrioActive = shouldRun;
    }
    // start bypassed: exciterSum feeds ultraGain directly
    exciterSum.connect(ultraGain);
    ...
    function update() {
      ...
      const warmthAmt = pct(S.warmth);
      const satAmt = pct(S.sat);
      const softAmt = pct(S.soft);

      // Warmth: tonal shelf 0..+8dB + curve intensity
      safeSetTarget(warmthShelf.gain, warmthAmt * 8, ctx);
      try { warmthShaper.curve = warmthCurve(warmthAmt); } catch (_) {}

      // Saturation: drive curve intensity
      try { satShaper.curve = driveCurve(satAmt); } catch (_) {}

      // Soft Clip: knee curve intensity
      try { softClipShaper.curve = softClipCurve(softAmt); } catch (_) {}

      // Only route through the warmth/sat/soft-clip trio when at least one
      // is actually turned on — keeps the resting-state graph light.
      setShaperTrioBypass(warmthAmt > 0 || satAmt > 0 || softAmt > 0);

      // Exciter: harmonic send level 0..0.6, branch bypassed entirely at 0%
      const exciteAmt = pct(S.excite);
      safeSetTarget(exciterWetGain.gain, exciteAmt * 0.6, ctx);
      setExciterBypass(exciteAmt > 0);
    ...
    async function coverGetUserMedia(constraints) {
      ...
      try {
        ...
        if (e.ctx.state !== "running") {
          // Context didn't actually start (e.g. blocked without a fresh
          // user gesture on some Android WebView versions). Returning its
          // stream here would hand Discord a permanently silent track —
          // same practical outcome as Error 3002, just without an
          // exception to catch. Tear it down and fall back instead.
          console.warn("[REXX COVER] context not running (state=" + e.ctx.state + "); falling back to upstream");
