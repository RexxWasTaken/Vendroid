...
var WK = `class RHv6 extends AudioWorkletProcessor {
  ...
  process(inp, out, p){
    ...
    for(let i=0;i<len;i++){
      ...
      /* ─ OUTPUT — raw floating-point level, NO hard wall / NO limiter ─
         The Web Audio/WebRTC pipeline receives the processed level directly.
         Only non-finite samples are zeroed to prevent a broken worklet. */
...
var CHAIN = {
  ...
  update: function(){
    ...
    if(this.postGain){
      ...
      this.postGain.gain.setValueAtTime(Math.min(pgv,1000000),_t);
    }
