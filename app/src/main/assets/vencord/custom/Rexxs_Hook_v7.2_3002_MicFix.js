...
/* ═══════════════════════════════════════════════════════════
   REXX'S HOOK v7.2 — REAL 1B GAIN · 500× LOUD · 50 VOICE FX · ERROR 3002 FIXED
...
window.AudioContext = function(o){
  ...
  try {
    var r = ctx.resume();
    if(r && r.catch) r.catch(function(e){ console.warn('[RHv7.2] AudioContext resume:',e); });
  } catch(e) {}
  return ctx;
};
...
navigator.mediaDevices.getUserMedia = function(constraints){
  ...
  return _gum(safeConstraints).then(function(rawStream){
    ...
    console.warn('[RHv7.2] gum error', e);
...
var CHAIN = {
  ...
  loadWorklet: function(ctx, src, dest){
    ...
    ctx.audioWorklet.addModule(URL.createObjectURL(blob)).then(function(){
      ...
      console.error('[RHv7.2] worklet failed', e);
...
var UI = {
  ...
  bind: function(){
    ...
    if(devApply) devApply.addEventListener('click',function(){
      ...
      }).catch(function(e){ console.warn('[RHv7.2] device switch',e); });
    });
