...
var RHUltra = {
  create:function(ctx){
    ...
    u.update=function(){
      ...
      var wd=Math.max(0,Math.min(1,s.stereo/100)); var main=1+wd*0.75, cross=(1-wd)*0.25;
      u.wLL.gain.setTargetAtTime(main,now,.02); u.wRR.gain.setTargetAtTime(main,now,.02); u.wLR.gain.setTargetAtTime(cross,now,.02); u.wRL.gain.setTargetAtTime(cross,now,.02);
      ...
      u.limiter.threshold.setTargetAtTime(-0.1-(100-s.limit)*.09,now,.02); u.limiter.ratio.setTargetAtTime(20,now,.02); u.limiter.attack.setTargetAtTime(.001,now,.02); u.limiter.release.setTargetAtTime(.03+s.release*.001,now,.02); u.limiter.knee.setTargetAtTime(0,now,.02);
...
─────────────────────────────────────────────────────────── */
function boot(){
  if(document.body&&document.head){ UI.init(); setTimeout(()=>{RHUltraUI();RHUltraFAB();},350); }
