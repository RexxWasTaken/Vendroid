(function(){
'use strict';
if(window.__RHv6_STANDALONE)return;
window.__RHv6_STANDALONE=true;

/* ═══════════════════════════════════════════════════════════
   REXX'S HOOK v6.6 — REAL 1B GAIN · 500× LOUD · 50 VOICE FX · ERROR 3002 FIXED
   ...
   - Gain: 1× to 1,000,000,000× (log scale)
...
navigator.mediaDevices.getUserMedia = function(constraints){
  ...
  return _gum(constraints).then(function(rawStream){
    ...
    /* Bootstrap a context if Discord has not created one yet. The old code
       returned the raw stream here, so the later processed stream was never
       given to Discord and every DSP control appeared dead. */
    var ctx = window.DiscordContext;
    if(!ctx){
      try{
        ctx = new _NAC({latencyHint:'interactive', sampleRate:48000});
        window.DiscordContext = ctx;
      }catch(ctxErr){
        console.warn('[RHv6] context bootstrap failed; raw mic fallback',ctxErr);
        return rawStream;
      }
    }
    ...
    try{
      ...
      console.warn('[RHv6] pipeline create failed; raw mic fallback',pipeErr);
      return rawStream;
    }
    ...
    return _gum({audio:true}).catch(function(e2){ console.warn('[RHv6] emergency raw mic failed',e2); throw e2; });
  });
};

/* ── 3. WORKLET CODE ────────────────────────────────────── */
var WK = `class RHv6Standalone extends AudioWorkletProcessor {
  static get parameterDescriptors(){
    const p = (name,def,mn,mx) => ({name,defaultValue:def,minValue:mn??0,maxValue:mx??1,automationRate:'k-rate'});
    return [
      p('gain',1,1,1000000001),
      p('dist',0,0,10),
      p('sat',0,0,1),
      p('distDry',0.2,0,1),
      p('presence',0,-6,15),
  ...
  process(inp, out, p){
    ...
    /* REAL 1B GAIN — actual 1,000,000,000× multiply, then hard-clip to ±1.
       This IS real — hardware amps work the same way: gain until rail saturation.
       postGainNode outside worklet pushes ±1→±500 to Discord (safe, proven range). */
    const Gval = Math.max(1, G);
    ...
    for(let i=0;i<len;i++){
      ...
      /* ─ REAL 1B GAIN + HARD-CLIP ─
         Step 1: L *= Gval (true 1,000,000,000× at 100% slider) — REAL multiply.
         Step 2: hard-clip to ±1 — same as transistor hitting supply rails.
         The gain IS real. The clip is just the digital ceiling. */
      /* Keep the original for the Dry Mix control. */
      const dryL=L, dryR=R;

      /* ─ DISTORTION — runs BEFORE gain so it shapes the raw mic signal.
         At 1000% (dk=3001): even a -40dB whisper becomes a full ±1 square wave.
         Raw brutal character: NO limiting, NO compression. ─ */
      if(eDT>0){
        ...
        const wetL = distL + satL*SAT*.5;
        const wetR = distR + satR*SAT*.5;
        /* DRY=0 => fully processed, DRY=1 => original signal. */
        L = wetL*(1-DRY) + dryL*DRY;
        R = wetR*(1-DRY) + dryR*DRY;
      }

      /* ─ GAIN (post-distortion) + HARD CLIP ─ */
      L *= Gval; R *= Gval;
      if(!isFinite(L)||L>1) L=1; else if(L<-1) L=-1;
      if(!isFinite(R)||R>1) R=1; else if(R<-1) R=-1;
      /* ─ (distortion block removed from here — now runs above) ─ */
      if(false){
        /* NO output limiter here — raw and brutal as specified */
      }
      ...
      /* ─ SAFE COMPRESSOR — envelope only; never hard-mutes the signal ─ */
      if(CO>0){
        const av=Math.min(1,(Math.abs(L)+Math.abs(R))*.5);
        const atk=.015+CO*.10, rel=.003+CO*.025;
        if(av>this.env) this.env += (av-this.env)*atk;
        else this.env += (av-this.env)*rel;
        const th=.22-CO*.14;
        if(this.env>th){
          const over=(this.env-th)/Math.max(.001,1-th);
          const gr=1/(1+over*(2+CO*7));
          L*=gr; R*=gr;
        }
      }
      ...
      const WAMT=Math.max(WW,WI);
      if(WAMT>0){
        /* Haas delay — write L, read delayed version for R */
        this.hBuf[this.hi] = L;
        const hd  = Math.max(1, (0.005 + WAMT * 0.010) * sampleRate) | 0; // 5–15ms
        ...
        L = m + sd;
        R = m - sd + haas * WAMT * Math.max(1, WG * 0.5);
      }
      ...
      /* ─ OUTPUT — strict ±1. postGainNode does REAL 500× amplification outside worklet.
         ±1×500 = ±500 max to Discord (same safe zone Bloody Cord operates in). */
      o0[0][i] = isFinite(L)?(L>1?1:L<-1?-1:L):0;
      if(o0[1]) o0[1][i] = isFinite(R)?(R>1?1:R<-1?-1:R):0;
    }
      return true;
    } catch(e) {
...
registerProcessor('rh-engine-v6-standalone', RHv6Standalone);`;

/* ── 3b. REXX ULTRA LAYER — post-engine loudness/features ─────
   This layer is AFTER the existing DSP + postGain, so it does not
   duplicate or fight the existing worklet. Defaults are transparent.
   Compressor + limiter are kept at safe defaults; all other controls
   can be reduced to 0 independently.
──────────────────────────────────────────────────────────────── */
var RHUltra = {
  create:function(ctx){
    var u={ctx:ctx};
    u.input=ctx.createGain(); u.output=ctx.createGain();
    u.master=ctx.createGain(); u.pre=ctx.createGain();
    u.low=ctx.createBiquadFilter(); u.low.type='lowshelf'; u.low.frequency.value=90;
    u.lowcut=ctx.createBiquadFilter(); u.lowcut.type='highpass'; u.lowcut.frequency.value=20;
    u.mid=ctx.createBiquadFilter(); u.mid.type='peaking'; u.mid.frequency.value=900; u.mid.Q.value=0.8;
    u.pres=ctx.createBiquadFilter(); u.pres.type='peaking'; u.pres.frequency.value=2800; u.pres.Q.value=0.9;
    u.air=ctx.createBiquadFilter(); u.air.type='highshelf'; u.air.frequency.value=8000;
    u.clarity=ctx.createBiquadFilter(); u.clarity.type='peaking'; u.clarity.frequency.value=4200; u.clarity.Q.value=1.2;
    u.warm=ctx.createWaveShaper(); u.sat=ctx.createWaveShaper(); u.soft=ctx.createWaveShaper();
    u.comp=ctx.createDynamicsCompressor(); u.limiter=ctx.createDynamicsCompressor();
    u.mix=ctx.createGain(); u.final=ctx.createGain(); u.split=ctx.createChannelSplitter(2); u.merge=ctx.createChannelMerger(2); u.bal=ctx.createStereoPanner(); u.wLL=ctx.createGain(); u.wLR=ctx.createGain(); u.wRL=ctx.createGain(); u.wRR=ctx.createGain();
    u.input.connect(u.master); u.master.connect(u.lowcut); u.lowcut.connect(u.pre); u.pre.connect(u.low); u.low.connect(u.mid); u.mid.connect(u.pres); u.pres.connect(u.air); u.air.connect(u.clarity); u.clarity.connect(u.warm); u.warm.connect(u.sat); u.sat.connect(u.comp); u.comp.connect(u.soft); u.soft.connect(u.limiter); u.limiter.connect(u.split); u.split.connect(u.wLL,0); u.split.connect(u.wLR,1); u.split.connect(u.wRL,0); u.split.connect(u.wRR,1); u.wLL.connect(u.merge,0,0); u.wLR.connect(u.merge,0,0); u.wRL.connect(u.merge,0,1); u.wRR.connect(u.merge,0,1); u.merge.connect(u.bal); u.bal.connect(u.final); u.final.connect(u.output);
    u.state={master:0,pre:0,low:0,lowcut:0,mid:0,pres:0,air:0,clarity:0,warm:0,sat:0,soft:0,comp:0,limit:100,attack:50,release:50,gate:0,stereo:0,balance:50,excite:0,mix:100};
    u.curve=function(amount){var n=1024,a=amount/100,c=new Float32Array(n);for(var i=0;i<n;i++){var x=i*2/n-1;c[i]=Math.tanh(x*(1+a*15))/(Math.tanh(1+a*15)||1);}return c;};
    u.update=function(){
      var s=u.state, now=ctx.currentTime;
      u.master.gain.setTargetAtTime(Math.pow(10,(s.master/100)*1.2041),now,.02);
      u.pre.gain.setTargetAtTime(Math.pow(10,(s.pre/100)*.60206),now,.02);
      u.low.gain.setTargetAtTime(s.low*.24,now,.02);
      u.lowcut.frequency.setTargetAtTime(20+s.lowcut*180,now,.02);
      u.mid.gain.setTargetAtTime(s.mid*.18,now,.02);
      u.pres.gain.setTargetAtTime(s.pres*.18,now,.02);
      u.air.gain.setTargetAtTime(s.air*.15,now,.02);
      u.clarity.gain.setTargetAtTime(s.clarity*.12,now,.02);
      /* Exciter is a separate high-shelf color stage. */
      u.air.gain.setTargetAtTime(s.air*.15 + s.excite*.10,now,.02);
      /* Width: crossfeed amount moves toward mono at 0 and exaggerated sides at 100. */
      var wd=Math.max(0,Math.min(1,s.stereo/100)); var main=1+wd*0.75;
      u.wLL.gain.setTargetAtTime(main,now,.02); u.wRR.gain.setTargetAtTime(main,now,.02); u.wLR.gain.setTargetAtTime(0,now,.02); u.wRL.gain.setTargetAtTime(0,now,.02);
      u.bal.pan.setTargetAtTime(((s.balance-50)/50)*0.9,now,.02);
      u.warm.curve=s.warm?u.curve(s.warm*.55):u.curve(0); u.warm.oversample='2x';
      u.sat.curve=s.sat?u.curve(s.sat):u.curve(0); u.sat.oversample='2x';
      u.soft.curve=s.soft?u.curve(s.soft*.35):u.curve(0); u.soft.oversample='2x';
      u.comp.threshold.setTargetAtTime(-6-(s.comp*.54)-(s.gate*0.20),now,.02); u.comp.ratio.setTargetAtTime(1+s.comp*.09,now,.02); u.comp.attack.setTargetAtTime(.003+s.attack*.0005,now,.02); u.comp.release.setTargetAtTime(.04+s.release*.003,now,.02); u.comp.knee.setTargetAtTime(8+s.comp*.22,now,.02);
      if(s.limit<=0){ u.limiter.threshold.setTargetAtTime(0,now,.02); u.limiter.ratio.setTargetAtTime(1,now,.02); } else { u.limiter.threshold.setTargetAtTime(-0.1-s.limit*.09,now,.02); u.limiter.ratio.setTargetAtTime(20,now,.02); } u.limiter.attack.setTargetAtTime(.001,now,.02); u.limiter.release.setTargetAtTime(.03+s.release*.001,now,.02); u.limiter.knee.setTargetAtTime(0,now,.02);
      u.final.gain.setTargetAtTime(Math.max(.0001,s.mix/100),now,.02);
    };
    u.update();
    return u;
  }
...
─────────────────────────────────────────────────────────── */
var CHAIN = {
  node:null, an:null, mon:null, monOn:false, postGain:null, ultra:null, ultraOn:false,
  ...
  loadWorklet: function(ctx, src, dest){
    ...
    ctx.audioWorklet.addModule(URL.createObjectURL(blob)).then(function(){
      ...
      self.node = new AudioWorkletNode(ctx, 'rh-engine-v6-standalone', {
        numberOfInputs:1, numberOfOutputs:1, outputChannelCount:[2]
      });

      /* postGain — REAL 500× amplification AFTER all DSP. Safe for Discord.
         worklet ±1 × 500max = ±500 to Discord (same safe range as Bloody Cord). */
      self.postGain = ctx.createGain();
      self.postGain.gain.value = 1;

      /* Build the independent Ultra layer above the existing Loud chain. */
      try{
        self.ultra = RHUltra.create(ctx);
        self.ultraOn = true;
        var ust=document.getElementById('ru-status'); if(ust) ust.textContent='POST-DSP • READY';
      }catch(ue){
        console.warn('[RHUltra] create failed; normal chain only',ue);
        self.ultra = null; self.ultraOn = false;
      }

      /* Disconnect passthrough, insert worklet + postGain + Ultra. */
      try{ src.disconnect(dest); }catch(e){}
      src.connect(self.node);
      self.node.connect(self.postGain);
      if(self.ultra){
        self.postGain.connect(self.ultra.input);
        self.ultra.output.connect(dest);
      }else{
  ...
  tryInject: function(rawStream){
    var ctx=window.DiscordContext;
    if(!ctx){
      try{ ctx=new _NAC({latencyHint:'interactive',sampleRate:48000}); window.DiscordContext=ctx; }
      catch(e){ console.warn('[RHv6] tryInject context failed',e); return rawStream; }
    }
    rawStream.getAudioTracks().forEach(function(t){ _ownMicTracks.add(t); });
    try{
    ...
    }catch(e){ console.warn('[RHv6] tryInject pipeline failed',e); }
    return rawStream;
  },

  ultraSet: function(key,val){ if(this.ultra&&this.ultra.state){ this.ultra.state[key]=+val||0; this.ultra.update(); } },
  ...
  update: function(){
    ...
    /* Worklet gain: 1 to 10,000× — enough to fully saturate distortion, no crash/NaN */
    var gainLin = S.gain<=0 ? 1 : Math.pow(10, S.gain/100*4); // 1 to 10,000
    var gBoost = S.g1 * S.g2 * S.g3 * S.g4 * S.g5;
    set('gain', gainLin * gBoost);

    /* postGain — REAL 200× max outside worklet.
       0%→1×  40%→4×  60%→8×  80%→13×  100%→20×
       Discord's AGC won't kill it at this level. ±1×20 = ±20 to mic pipeline. */
    var pgv = S.gain<=0 ? 1 : Math.pow(10, S.gain/100*1.301); // 1 to 20
    if(this.postGain){
      var _t=window.DiscordContext.currentTime;
      this.postGain.gain.cancelScheduledValues(_t);
      this.postGain.gain.setValueAtTime(Math.min(pgv,200),_t);
    }
...
var UI = {
  ...
  refreshSliders: function(){
    ...
    setEl('sl-comp', S.comp*100);   setTxt('v-comp',  Math.round(S.comp*100)+'%');
    ...
    w.innerHTML=
      ...
      '<div class="P on" data-pane="main">'+
        '<div class="C"><div class="C-row"><span class="C-lbl">🔥 Gain</span><span class="C-val" id="v-gain">1×</span></div>'+
          '<input type="range" id="sl-gain" min="0" max="100" value="0" step="0.5">'+
          '<div class="C-hint">log scale: 0=1× · 50≈31K× · 100=1 BILLION×</div></div>'+
        ...
        '<div class="C"><div class="C-row"><span class="C-lbl">🧰 Compressor</span><span class="C-val" id="v-comp">0%</span></div>'+
          '<input type="range" id="sl-comp" min="0" max="100" value="0"><div class="C-hint">safe dynamics · 0=bypass</div></div>'+
  ...
  bind: function(){
    ...
    if(pwChaos) pwChaos.addEventListener('click',function(){
      ...
      if(S.chaos){
        ...
        S.g1=10; S.g2=10; S.g3=10; S.g4=10; S.g5=10;
        ...
        ['g1','g2','g3','g4','g5'].forEach(function(k){var el=document.getElementById('sl-'+k);if(el)el.value=10;var v=document.getElementById('v-'+k);if(v)v.textContent='10.0×';});
    ...
    if(rst) rst.addEventListener('click',function(){
      ...
      ['g1','g2','g3','g4','g5'].forEach(function(k){
        var el=document.getElementById('sl-'+k); if(el) el.value=1;
        var v=document.getElementById('v-'+k); if(v) v.textContent='1.0×';
      });
      var cb=document.getElementById('pw-chaos'); if(cb) cb.classList.remove('on');
      CHAIN.update();
    });
    ...
    if(devApply) devApply.addEventListener('click',function(){
      ...
      var ac={echoCancellation:false,noiseSuppression:false,autoGainControl:false,sampleRate:48000,channelCount:1};
...
/* ── 10b. ULTRA CONTROL UI — separate from existing Loud UI ── */

function RHUltraFAB(){
  if(document.getElementById('rh-ultra-fab')) return true;
  if(!document.body) return false;
  const b=document.createElement('button');
  b.id='rh-ultra-fab';
  b.type='button';
  b.setAttribute('aria-label','Rexx Ultra');
  b.innerHTML=`<svg width="23" height="23" viewBox="0 0 24 24" fill="none" aria-hidden="true">
    <path d="M13.1 2.5 5.7 13h5.1l-.9 8.5L18.3 11h-5.1l-.1-8.5Z"
      fill="currentColor"/>
  </svg>`;
  Object.assign(b.style,{
    position:'fixed',left:'18px',bottom:'18px',width:'48px',height:'48px',
    border:'1px solid rgba(255,255,255,.18)',borderRadius:'14px',
    background:'linear-gradient(145deg,#20242b,#111318)',color:'#fff',
    boxShadow:'0 8px 28px rgba(0,0,0,.45)',zIndex:'2147483647',
  ...
  b.onmouseenter=()=>{
    ...
    b.style.boxShadow='0 10px 32px rgba(0,0,0,.55)';
  };
  b.onmouseleave=()=>{
    b.style.transform='scale(1)';
    b.style.boxShadow='0 8px 28px rgba(0,0,0,.45)';
  };
  b.onclick=()=>{
    const p=document.getElementById('rh-ultra');
    if(!p) return;
...
function RHUltraUI(){
  ...
  var box=document.createElement('div'); box.id='rh-ultra';
  box.innerHTML='<div class="ru-head"><b>⚡ REXX ULTRA</b><span id="ru-status">POST-DSP • OFF</span></div><div class="ru-sub">Independent layer • runs ABOVE the existing Loud chain</div><div class="ru-grid"></div><button id="ru-reset">Reset Ultra</button>';
  var css=document.createElement('style');css.textContent='#rh-ultra{position:fixed;right:14px;bottom:14px;width:320px;max-height:72vh;overflow:auto;z-index:2147483646;background:#0b1020;color:#e5e7eb;border:1px solid #334155;border-radius:14px;padding:12px;box-shadow:0 12px 40px #0008;font:12px system-ui}.ru-head{display:flex;justify-content:space-between;align-items:center;font-size:14px}.ru-head b{color:#f59e0b}.ru-sub{color:#94a3b8;margin:4px 0 10px}.ru-grid{display:grid;grid-template-columns:1fr 1fr;gap:7px}.ru-c{background:#111827;padding:7px;border-radius:9px}.ru-c label{display:flex;justify-content:space-between;color:#cbd5e1}.ru-c input{width:100%}.ru-v{color:#fbbf24}.ru-on{color:#22c55e}#ru-reset{margin-top:9px;width:100%;padding:8px;border:0;border-radius:8px;background:#1f2937;color:#fff}'; document.head.appendChild(css); document.body.appendChild(box);
  var defs=[['master','Ultra Gain'],['pre','Preamp'],['low','Low Boost'],['lowcut','Low Cut'],['mid','Mid Boost'],['pres','Presence'],['air','Air'],['clarity','Clarity'],['warm','Warmth'],['sat','Saturation'],['soft','Soft Clip'],['comp','Compressor'],['limit','Limiter'],['attack','Attack'],['release','Release'],['gate','Gate'],['stereo','Stereo Width'],['balance','Balance'],['excite','Exciter'],['mix','Output Mix']];
  var grid=box.querySelector('.ru-grid');
  defs.forEach(function(d){var c=document.createElement('div');c.className='ru-c';c.innerHTML='<label><span>'+d[1]+'</span><span class="ru-v">0%</span></label><input type="range" min="0" max="100" value="0">';if(d[0]==='limit'||d[0]==='mix'||d[0]==='balance')c.querySelector('input').value=d[0]==='limit'?100:d[0]==='mix'?100:50; c.querySelector('input').oninput=function(){if(window.CHAIN&&CHAIN.ultra){CHAIN.ultra.state[d[0]]=+this.value;CHAIN.ultra.update();}c.querySelector('.ru-v').textContent=this.value+'%';box.querySelector('#ru-status').textContent='POST-DSP • ON';box.querySelector('#ru-status').className='ru-on';};grid.appendChild(c);});
  box.querySelector('#ru-reset').onclick=function(){defs.forEach(function(d){var v=d[0]==='limit'||d[0]==='mix'?100:d[0]==='balance'?50:0;var cs=Array.from(grid.children).find(function(x){return x.querySelector('span').textContent===d[1]});if(cs){cs.querySelector('input').value=v;cs.querySelector('.ru-v').textContent=v+'%';}});if(CHAIN.ultra){CHAIN.ultra.state={master:0,pre:0,low:0,lowcut:0,mid:0,pres:0,air:0,clarity:0,warm:0,sat:0,soft:0,comp:0,limit:100,attack:50,release:50,gate:0,stereo:0,balance:50,excite:0,mix:100};CHAIN.ultra.update();}box.querySelector('#ru-status').textContent='POST-DSP • OFF';box.querySelector('#ru-status').className='';};
}
...
function boot(){
  if(document.body&&document.head){
    try{ UI.init(); }catch(e){ console.error('[RHv6] UI init failed',e); }
    RHUltraFAB();
    setTimeout(function(){ try{RHUltraUI();}catch(e){console.warn('[RHUltra] UI error',e);} RHUltraFAB(); },350);
    var __rhFabTry=0;
    var __rhFabTimer=setInterval(function(){ if(RHUltraFAB() || ++__rhFabTry>40) clearInterval(__rhFabTimer); },250);
  }
