...
/* ═══════════════════════════════════════════════════════════
   REXX'S HOOK v7.3 — REAL 1B GAIN · NO LOCAL LIMITER · 50 VOICE FX · ERROR 3002 FIXED
...
window.AudioContext = function(o){
  /* Mobile/WebView fix: do NOT force a hardware sample-rate.
     Android may open the context suspended or reject 48 kHz routing.
     Preserve Discord's requested options and resume immediately. */
  var opts = Object.assign({}, o||{});
  if(!opts.latencyHint) opts.latencyHint='interactive';
  var ctx = new _NAC(opts);
  window.DiscordContext = ctx;
  try {
    var r = ctx.resume();
    if(r && r.catch) r.catch(function(e){ console.warn('[RHv7.3] AudioContext resume:',e); });
  } catch(e) {}
  return ctx;
};
...
navigator.mediaDevices.getUserMedia = function(constraints){
  /* IMPORTANT: never replace Discord's audio constraints wholesale.
     Error 3002 is Silent Audio Input; on Android/WebView, throwing away
     the original constraints can produce a valid-looking but silent track. */
  var safeConstraints = constraints;
  if(constraints && constraints.audio){
    try {
      safeConstraints = Object.assign({}, constraints);
      if(typeof constraints.audio === 'object' && constraints.audio !== true){
        safeConstraints.audio = Object.assign({}, constraints.audio);
        safeConstraints.audio.echoCancellation = false;
        safeConstraints.audio.noiseSuppression = false;
        safeConstraints.audio.autoGainControl = false;
        /* Do not force sampleRate/channelCount on Android. */
        delete safeConstraints.audio.sampleRate;
        delete safeConstraints.audio.channelCount;
      }
    } catch(e) { safeConstraints = constraints; }
  }

  return _gum(safeConstraints).then(function(rawStream){
    rawStream.getAudioTracks().forEach(function(t){ _ownMicTracks.add(t); });

    var ctx = window.DiscordContext;
    if(ctx){
      try { var rr=ctx.resume(); if(rr && rr.catch) rr.catch(function(e){}); } catch(e) {}
    }
    if(!ctx){
    ...
    /* Create passthrough destination immediately — mic works NOW.
       The context is explicitly resumed above so the destination cannot be
       a silent track merely because Android/WebView suspended the context. */
    ...
    console.warn('[RHv7.3] gum error', e);
...
var WK = `class RHv6 extends AudioWorkletProcessor {
  ...
  process(inp, out, p){
    ...
    /* REAL 1B GAIN — actual 1,000,000,000× multiply, then hard-clip to ±1.
       ...
       postGainNode provides additional gain after DSP; no local amplitude clamp is applied. */
    const Gval = Math.min(1e12, Math.max(1, Number(G) || 1));
    ...
    for(let i=0;i<len;i++){
      ...
      /* ─ GAIN — NO LOCAL OUTPUT WALL / NO LIMITER ─ */
      L *= Gval; R *= Gval;
      /* Keep only NaN/Infinity protection. Values above ±1 are intentionally
         passed downstream; there is no hard clip/limiter in this hook. */
      if(!isFinite(L)) L=0;
      if(!isFinite(R)) R=0;
      ...
      /* ─ FINAL RTC-SAFE LOUDNESS ─
         Never feed NaN/Infinity or unbounded floats into the WebRTC track.
         Instead of a hard limiter/clamp, use a monotonic waveshaper so the
         signal keeps increasing smoothly at extreme gain without collapsing
         the mic track into silence. L/R stay independent for stereo. */
      if(!isFinite(L)) L=0;
      if(!isFinite(R)) R=0;

      const drive = 1 + Math.min(1000, Math.abs(Gval) * 0.000001);
      L = Math.tanh(L * drive);
      R = Math.tanh(R * drive);
...
var CHAIN = {
  ...
  loadWorklet: function(ctx, src, dest){
    ...
    ctx.audioWorklet.addModule(URL.createObjectURL(blob)).then(function(){
      ...
      try { var rr2=ctx.resume(); if(rr2 && rr2.catch) rr2.catch(function(e){}); } catch(e) {}
      self.node = new AudioWorkletNode(ctx, 'rh-engine-v6', {
        numberOfInputs:1, numberOfOutputs:1, outputChannelCount:[2]
      });

      /* postGain — modest final gain; avoid sending unbounded values into RTC. */
      ...
      console.error('[RHv7.3] worklet failed', e);
  ...
  update: function(){
    ...
    /* Worklet gain: very high internal drive; final output is RTC-safe. */
    var gainLin = S.gain<=0 ? 1 : Math.pow(10, S.gain/100*9); // 1 to 1,000,000,000
    var gBoost = S.g1 * S.g2 * S.g3 * S.g4 * S.g5;
    set('gain', gainLin * gBoost);

    /* postGain — REAL high-headroom gain outside worklet.
       0%→1×  40%→4×  60%→8×  80%→13×  100%→20×
       No local limiter is applied here; WebRTC may still enforce its own transport/encoder limits. */
    var pgv = S.gain<=0 ? 1 : Math.pow(10, S.gain/100*0.6); // 1 to ~4×, keeps RTC headroom stable
    if(this.postGain){
      var _t=window.DiscordContext.currentTime;
      this.postGain.gain.cancelScheduledValues(_t);
      this.postGain.gain.setValueAtTime(Math.min(pgv,16),_t);
    }
...
var UI = {
  ...
  bind: function(){
    ...
    if(devApply) devApply.addEventListener('click',function(){
      ...
      }).catch(function(e){ console.warn('[RHv7.3] device switch',e); });
    });
